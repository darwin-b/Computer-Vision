...
def transform(im, T):
    ...
    im_new = np.zeros(im.shape,dtype=int)
    ...
    for i in range(r):
        for j in range(c):
            ...
            y,x,_ = np.matmul(inv,np.array([j,i,1]))

            # print(x,y)
            scale_x = x/_
            scale_y = y/_
            if 0<=scale_x<r and 0<=scale_y<c:
                # print(im[int(scale_x),int(scale_y)])
                # break
                im_new[i,j,:] = im[int(scale_x),int(scale_y)]
            
                # im_new[pixelTransorm] = color
    # im_new = np.dot(im,T)
    return im_new

# i,j = np.dot(pinv(T),im)
